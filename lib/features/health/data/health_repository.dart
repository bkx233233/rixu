...
class FoodItem {
  const FoodItem(
      ...
      this.needsNutritionCompletion = false});
  final String id;
  final String name;
  ...
  NutritionValues calculate(double grams) {
    ...
    return NutritionValues(
        ...
        carbohydrate: carbohydratePer100g! * factor,
...
class NutritionValues {
  ...
  final double fat;
  static const zero =
      NutritionValues(calories: 0, protein: 0, carbohydrate: 0, fat: 0);
...
class WeightPoint {
  const WeightPoint(
      {required this.date, required this.weight, this.recordedAt});
...
class HealthDayData {
  ...
  NutritionValues get intake => meals.fold(
...
class HealthRepository {
  ...
  Future<HealthDayData> loadDay(DateTime date) async {
    ...
    final profileRequest = _client
        .from('profiles')
        .select('default_fitness_status')
        .eq('id', _userId)
        .maybeSingle();
    final statusRequest = _client
        .from('fitness_day_statuses')
        .select('status')
        .eq('user_id', _userId)
        .eq('local_date', day)
        .maybeSingle();
    final weightRequest = _client
        .from('body_weight_entries')
        .select('weight_kg')
        .eq('user_id', _userId)
        .eq('local_date', day)
        .maybeSingle();
    final mealRequest = _client
        .from('meal_entries')
        .select(
    ...
    return HealthDayData(
        fitnessStatus: status?['status'] as String? ??
            profile?['default_fitness_status'] as String? ??
            'rest',
        weight: (weight?['weight_kg'] as num?)?.toDouble(),
        meals: meals.map(_mealFromRow).toList(),
        exercises: exercises.map(_exerciseFromRow).toList());
  }

  Future<ProfileData> loadProfile() async {
    final row = await _client
        .from('profiles')
        .select('height_cm,sex,birth_date,activity_level,onboarding_completed')
        .eq('id', _userId)
        .single();
    return ProfileData(
  ...
  Future<NutritionTarget?> loadNutritionTarget() async {
    final row = await _client
        ...
        .select(
            'calorie_kcal,protein_g,carbohydrate_g,fat_g,daily_calorie_deficit_kcal,calculation_mode')
        .eq('user_id', _userId)
        .maybeSingle();
    ...
    return NutritionTarget(
        ...
        carbohydrate: (row['carbohydrate_g'] as num?)?.toDouble() ?? 0,
        fat: (row['fat_g'] as num?)?.toDouble() ?? 0,
        deficit: (row['daily_calorie_deficit_kcal'] as num?)?.toInt() ?? 0,
        mode: row['calculation_mode'] as String? ?? 'calculated');
  }
  ...
  Future<void> saveProfileAndTarget(
      ...
      required NutritionTarget target}) async {
    await Future.wait([
  ...
  Future<void> saveNutritionTarget(NutritionTarget target) =>
      _client.from('nutrition_targets').upsert({
        'user_id': _userId,
        'calorie_kcal': target.calories.round(),
        'protein_g': target.protein,
        'carbohydrate_g': target.carbohydrate,
        'fat_g': target.fat,
        'daily_calorie_deficit_kcal': target.deficit,
        'calculation_mode': target.mode
      });
  ...
  Future<List<FoodItem>> loadFoodItems() async {
    final results = await Future.wait([
      _client
          .from('food_items')
          .select(
              'id,name,category,calories_per_100g,protein_per_100g,carbohydrate_per_100g,fat_per_100g,source_name')
          .eq('user_id', _userId)
          .eq('is_active', true)
          .order('name'),
      _client
          .from('system_food_items')
          .select(
              'id,name,category,calories_per_100g,protein_per_100g,carbohydrate_per_100g,fat_per_100g,source_name')
          .eq('is_active', true)
          .order('name'),
    ...
    return [
      ...List<Map<String, dynamic>>.from(results[0] as List)
          .map((row) => _foodFromRow(row, false)),
      ...List<Map<String, dynamic>>.from(results[1] as List)
          .map((row) => _foodFromRow(row, true))
  ...
  Future<void> saveFoodItem(
    ...
    final per100Carbohydrate =
        carbohydratePer100g ?? (carbohydrate ?? 0) * factor;
    ...
    final values = {
      ...
      'default_amount': 100,
      'default_unit': 'g',
      'calorie_kcal': per100Calories,
      'protein_g': per100Protein,
      'carbohydrate_g': per100Carbohydrate,
      'fat_g': per100Fat,
      'calories_per_100g': per100Calories,
      'protein_per_100g': per100Protein,
      'carbohydrate_per_100g': per100Carbohydrate,
      'fat_per_100g': per100Fat,
      'source_name': '个人录入',
      'is_active': true
    };
    if (id == null) {
      await _client.from('food_items').insert(values);
    } else {
      await _client
          .from('food_items')
          .update(values)
  ...
  Future<void> deleteFoodItem(String id) =>
      _client.from('food_items').delete().eq('id', id).eq('user_id', _userId);
  Future<void> setFitnessStatus(
          {required DateTime date, required String status}) =>
      _client.from('fitness_day_statuses').upsert(
          {'user_id': _userId, 'local_date': _dateOnly(date), 'status': status},
          onConflict: 'user_id,local_date');
  Future<void> saveWeight({required DateTime date, required double weight}) =>
      _client.from('body_weight_entries').upsert({
        'user_id': _userId,
        'local_date': _dateOnly(date),
        'weight_kg': weight
      }, onConflict: 'user_id,local_date');
  Future<void> deleteWeight(DateTime date) => _client
      .from('body_weight_entries')
      .delete()
      .eq('user_id', _userId)
      .eq('local_date', _dateOnly(date));
  ...
  Future<void> addMeal(
          ...
          required double fat,
          String? foodItemId}) =>
      _client.from('meal_entries').insert({
        'user_id': _userId,
        ..._mealValues(
            date: date,
            mealType: mealType,
  ...
  Future<void> deleteMeal(String id) =>
      _client.from('meal_entries').delete().eq('id', id).eq('user_id', _userId);
  ...
  Future<void> addExercise(
    ...
    final sessions = List<Map<String, dynamic>>.from(await _client
        .from('workout_sessions')
        .select('id')
        .eq('user_id', _userId)
        .eq('local_date', day)
  ...
  Future<void> updateExercise(
          ...
          double? weight,
          int? durationSeconds}) =>
      _client.from('workout_exercise_logs').update({
        'exercise_name': name,
        'sets_count': sets,
        'repetitions': repetitions,
        'weight_kg': weight,
  ...
  Future<void> deleteExercise(String id) =>
      _client.from('workout_exercise_logs').delete().eq('id', id);

  Map<String, dynamic> _mealValues(
          {required DateTime date,
          required String mealType,
          required String foodName,
          required double amount,
  ...
  MealEntry _mealFromRow(Map<String, dynamic> row) => MealEntry(
      id: row['id'] as String,
  ...
  WorkoutExercise _exerciseFromRow(Map<String, dynamic> row) => WorkoutExercise(
      id: row['id'] as String,
  ...
  FoodItem _foodFromRow(Map<String, dynamic> row, bool system) => FoodItem(
      id: row['id'] as String,
      name: row['name'] as String,
      category: row['category'] as String,
      caloriesPer100g: (row['calories_per_100g'] as num?)?.toDouble(),
      proteinPer100g: (row['protein_per_100g'] as num?)?.toDouble(),
      carbohydratePer100g: (row['carbohydrate_per_100g'] as num?)?.toDouble(),
      fatPer100g: (row['fat_per_100g'] as num?)?.toDouble(),
      isSystem: system,
      sourceName: row['source_name'] as String? ?? '个人录入',
      needsNutritionCompletion: !system && row['calories_per_100g'] == null);
